...
/* Embedded build (LegacyPadDisplay): hard cap on concurrent SOCKS sessions.
   ...
   otherwise exhaust memory. 48 is comfortable headroom for a single-user
   tunnel. */
#ifndef MAX_CLIENTS
...
int microsocks_main(int argc, char** argv) {
	...
	while(1) {
		collect(threads);
		/* Concurrency gate (embedded build): if we're already at the cap,
		   accept-and-drop the new connection so we don't queue unbounded
		   work or OOM on a memory-constrained device. */
		if(sblist_getsize(threads) >= MAX_CLIENTS) {
			struct client drop;
			if(!server_waitclient(&s, &drop))
				close(drop.fd);
